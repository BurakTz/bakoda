...
def test_list_rooms_returns_array(api_context: APIRequestContext):
    """GET /api/rooms returns a JSON array."""
    resp = api_context.get("/api/rooms")
    assert resp.ok
    data = resp.json()
...
def test_full_booking_flow(api_context: APIRequestContext):
    ...
    resp = api_context.get("/api/rooms?check_in=2027-03-01&check_out=2027-03-05")
    assert resp.ok
    rooms = resp.json()
...
def test_double_booking_conflict(api_context: APIRequestContext):
    ...
    resp = api_context.get("/api/rooms?check_in=2027-04-01&check_out=2027-04-03")
    assert resp.ok
    rooms = resp.json()
    assert len(rooms) > 0
    ...
    r1 = api_context.post("/api/bookings", data=payload)
    assert r1.status == 201

    r2 = api_context.post("/api/bookings", data={**payload, "guest_email": "second@test.com"})
