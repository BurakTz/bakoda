...
function App() {
  ...
  const loadData = () => {
    ...
    setLoading(true);
    Promise.all([
      fetch("/api/users/me/payment-methods", { headers: apiHeaders(token) }).then(r => r.ok ? r.json() : []),
      fetch("/api/users/me/billing-address", { headers: apiHeaders(token) }).then(r => r.ok ? r.json() : null),
    ])
